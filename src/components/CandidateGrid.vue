<template>
  <div>
    <table cellspacing="1" cellpadding="0">
      <tr v-for="r in dim">
        <td class="boardCell" v-for="c in dim">
          <candidate :digit="r * 3 + c + 1" :gridIndex="gridIndex" :index="index" :state="active[r][c]"/>         
        </td>
      </tr>
    </table>
  </div>
...
export default {
  ...
  props: ['candidate', 'gridIndex', 'index'],
  computed: {
    active () {
      let i, j, index
      let d = new Array(3)
      ...
      console.log(this.candidate, d)
      return d
    },
    dim () {
      return Array.from(Array(this.active.length), (v, k) => k)
    },
    gridId () {
...
#Wrapper
    background url('~statics/images/background.png') no-repeat top center
    min-height 720px
...
input[type="submit"]
    ...
    min-width 162px
    min-height 34px
    background url('~statics/images/submitBackground.png') repeat-x
    margin 8px 0px
    cursor pointer
...
input[type="submit"].gameControls
    ...
    min-width 28px
    min-height 28px
    background url('~statics/images/submitSmallBackground.png') repeat-x
    margin 8px 0px
    padding 2px 16px
    cursor pointer
...
#youWon
    ...
    top 0px
    right 0px
    background url('~statics/images/shade.png') repeat
